...
Floodfill::Floodfill(QObject *parent) : QObject(parent)
{
...
QImage Floodfill::floodfill(QImage *img, const QPoint &pos, const QRgb &newColor)
{
    QImage image = img->copy();

    //std::vector<QPoint> modified;
    QVector<QPoint> modified;

    int x = pos.x(), y = pos.y();
    const QRgb oldColor = image.pixel(x, y);

    //std::vector<fillpixelinfo> stack;
    QVector<fillpixelinfo> stack;
    ...
    if ((x >= 0) && (x < image.width()) && (y >= 0) && (y < image.height()))
    {
        ...
        while (!stack.empty())
        {
            ...
            for (x = x1; (x >= 0) && image.pixel(x, y) == oldColor; x--)
            {
                ...
                //modified.emplace_back(QPoint(x, y));
                modified.push_back(QPoint(x,y));
            }
            if (x >= x1)
            {
                goto skip;
